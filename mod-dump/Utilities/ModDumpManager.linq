	...
	public IEnumerable<ParsedMod> ReadUnpackedModFolders()
	{
		...
		foreach (DirectoryInfo siteFolder in this.GetSortedSubfolders(new DirectoryInfo(this.CachePath)))
		{
			if (!Enum.TryParse(siteFolder.Name, out ModSite site))
				throw new InvalidOperationException($"Invalid site folder in cache: {siteFolder.FullName} (folder name isn't a valid site key).");
			...
			foreach (DirectoryInfo modFolder in modFolders)
			{
				if (!long.TryParse(modFolder.Name, out long modId))
					throw new InvalidOperationException($"Invalid mod folder in cache: {siteFolder.FullName} (folder name isn't a valid mod ID).");
				...
				if (metadata is null)
					throw new InvalidOperationException($"Invalid mod folder in cache: {siteFolder.FullName} (no mod with ID {modId} found in cache)");
